...
class CustomArrayListTest {
	...
	void should_remove_items_at_index() {
		...
		addItems(sut,1000);
		sut.remove(0);
		sut.remove(20000);// Checking Index out of Bound Exception
		sut.get(3000);//Checking Index out of Bound Exception
		
		//Assert:
		assertEquals(2, sut.get(0));//Checking if the item: 1 is removed at index:0  -> Expected: 2 , Actual: ? 
		assertEquals(999, sut.getSize());//Checking if the length of the list is adjusting if we remove more items to it -> Expected: 999 , Actual: ?
	}
